...
| Amador post-02/15/2022 | Legacy PDFs/Wayback are automatic; routine daily live checks are disabled. | Current access appears portal/account-based rather than public static PDFs. | If there is a public unauthenticated portal path, automate it with Playwright in a virtual window; otherwise keep to legacy public archive/Wayback. |
...
| Riverside landing page | Automatic through a reader fallback for link discovery; source PDFs are still fetched directly from the court host. | Basic `requests` receives Cloudflare 403 on the landing page, while direct linked PDFs remain public. Chrome can clear the browser challenge after a short wait. | Keep the reader fallback scoped to discovery. If it breaks, use a browser/extension or VPS headed-browser scan to refresh the court-hosted PDF links. |
